...
export const KeyboardManager = () => {
  useEffect(() => {
    // Dynamic import for hotkeypad since it's a client-side library
    const initHotkeypad = async () => {
      try {
        const HotKeyPad = (await import('hotkeypad')).default;
        ...
        const data = profilesInfo.map(({ url, hotkey, icon, id, section, title }) => {
          return {
            id,
            title,
            icon,
  ...
  return (
    <>
      <div className="footer-button no-print">
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      <div id="hotkeypad" data-placeholder="Search Command"></div>
    </>
  );
};
